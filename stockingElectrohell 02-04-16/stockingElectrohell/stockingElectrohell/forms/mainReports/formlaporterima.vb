...
Public Class formlaporterima
    ...
    Private Sub formlaporterima_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        loadSize()
        loadcom()
    End Sub
    Private Sub loadcom()
        For Each a As String In clasc.retComplete("select namagarmen from tb_garment")
    ...
    Private Sub loadGrid()
        ...
        tempdt1 = clasg.loadFilteredDataTable("select a.kodeItem as ItemCode, b.nama,b.color, b.kodeSize as Kode ,a.qty as qtyTotal, b.hargaPokok" & _
                                              " from tb_produk b, tb_penerimaandetail a, tb_penerimaan c where a.kodeitem=b.kodeitem and b.status <> '' " & _
                                              "and a.kode=c.kode " & param & " group by a.kodeitem")

        tempdt2 = clasg.loadFilteredDataTable("SELECT kodeitem,value,sum(qty) as qty from tb_penerimaandetail inner join tb_penerimaan" & _
                                              " on tb_penerimaandetail.kode = tb_penerimaan.kode " & param2 & " group by kodeitem,value ")
    ...
    Private Sub rowTotalUpdate(value As DataRow, row As String)
        ...
        Catch ex As Exception
            MsgBox(value(11).ToString)
        End Try
        

        dGridAll.Rows(row).Cells(12).Value = Format(subtotal, "#,#0.00")
        formattotal = (Integer.Parse(dGridAll.Rows(row).Cells(13).Value) * subtotal)
        dGridAll.Rows(row).Cells(14).Value = Format(formattotal, "#,#0.00")
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If cbfaktur.Checked = True Then
            param = " where tb_penerimaandetail.kode = '" & txfaktur.Text & "'"
            

        ElseIf cbgarmen.Checked = True Then
            If txnama.Text = "" Then
                MsgBox("Nama Toko Masih Kosong")
            Else
                param = " and c.tgl between '" & tanggal.ToMySql & "' and '" & tanggal1.Year & "-" & _
                    tanggal1.Month & "-" & tanggal1.Day & " 23:59:59' "
                param2 = "where garmen = '" & txnama.Text & "' and tgl between '" & tanggal.ToMySql & "' and '" & tanggal1.Year & "-" & _
                    tanggal1.Month & "-" & tanggal1.Day & " 23:59:59' "
            End If
        End If
        dGridAll.Rows.Clear()
    ...
    Private Sub print()
        ...
        For Each dr As DataGridViewRow In Me.dGridAll.Rows
            dt.Rows.Add(tgl, txfaktur.Text, txnama.Text, kategori, txnama.Text, dr.Cells(0).Value, dr.Cells(1).Value, dr.Cells(2).Value, _
                        ...
                        dr.Cells(10).Value, dr.Cells(11).Value, dr.Cells(12).Value, dr.Cells(13).Value, dr.Cells(14).Value, dr.Cells(14).Value, _
