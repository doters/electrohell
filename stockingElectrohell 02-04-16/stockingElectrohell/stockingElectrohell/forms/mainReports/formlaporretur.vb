...
Public Class formlaporretur
    ...
    Dim kueri As String = ""
    Private Sub formlaporretur_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        loadSize()
    End Sub
    Private Sub loadcom()

        For Each a As String In clasc.retComplete(kueri)
    ...
    Private Sub loadGrid()
        ...
        tempdt1 = clasg.loadFilteredDataTable("select a.kodeItem as ItemCode, b.nama,b.color, b.kodeSize as Kode ,a.qty as qtyTotal, b.hargaPokok, b.hargaJual, b.status" & _
                                              " from tb_produk b, tb_returndetail a where a.kodeitem=b.kodeitem and b.status <> '' group by a.kodeitem")

        tempdt2 = clasg.loadFilteredDataTable("SELECT kodeitem,value,sum(qty) as qty from tb_returndetail inner join tb_return" & _
                                              " on tb_returndetail.kode = tb_return.kode " & param & _
    ...
    Private Sub rowTotalUpdate(value As DataRow, row As String)
        ...
        suntotalformat = (Integer.Parse(dGridAll.Rows(row).Cells(13).Value) * subtotal)
        dGridAll.Rows(row).Cells(14).Value = Format(suntotalformat, "#,#0.00")
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If cbfaktur.Checked = True Then
            param = " where tb_returndetail.kode = '" & txfaktur.Text & "'"
        ElseIf cbkategori.Checked = True Then
            If cmbcatjual.Text = "" Then
                MsgBox("Nama Toko Masih Kosong")
            Else
                param = " where tb_return.status = '" & cmbcatjual.Text & "' and tb_return.nama = '" & txnama.Text & "' and " & _
                    "tgl between '" & tanggal.ToMySql & "' and '" & tanggal1.Year & "-" & _
    ...
    Private Sub cmbcatjual_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbcatjual.SelectedIndexChanged
        If cmbcatjual.SelectedItem = "Store" Then
            lblnama.Text = "Store"
            kueri = "select namatoko from tb_stores"
            loadcom()

        Else
            lblnama.Text = "Warehouse"
            kueri = "select namagarmen from tb_garment"
            loadcom()
